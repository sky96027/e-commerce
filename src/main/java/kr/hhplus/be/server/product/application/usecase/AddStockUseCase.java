...
/**
 * [UseCase - Port In]
 * 상품 재고를 복구시키는 유스케이스에 대한 추상 정의.
 *
 * 이 인터페이스는 애플리케이션 계층에서
 * 재고를 증가시키는 기능을 외부(presentation, facade 등)에 노출하기 위한 계약(Contract)이다.
 *
...
public interface AddStockUseCase {
    /**
     * 옵션 ID와 판매량을 받아 재고를 증가시킨다.
     * @param optionId 옵션 ID
     * @param quantity 증가량
     */
    void addStock(long optionId, int quantity);
}
